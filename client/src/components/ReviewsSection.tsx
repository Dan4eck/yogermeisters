...
export default function ReviewsSection({ language }: ReviewsSectionProps): ReactElement {
  ...
  const handleSelect = (item: VideoTestimonial): void => {
    setSelected(item);
    ...
    <section id='reviews' className='relative overflow-hidden border-t border-white/5 bg-background py-14 lg:py-16'>
      ...
      <div className='container relative z-10 mx-auto px-6'>
        ...
        <motion.div
          ...
          className={cn('grid items-start gap-4', !isMobile && 'xl:grid-cols-[minmax(0,1.12fr)_330px]')}
        >
          {!isMobile ? (
            ...
            <div className='space-y-2'>
              {items.map((item: VideoTestimonial) => (
                <VideoPreview
                  key={item.id}
                  item={item}
                  isSelected={!isMobile && item.id === selected.id}
      ...
      <Dialog open={isMobileDialogOpen} onOpenChange={setIsMobileDialogOpen}>
        <DialogContent className='max-h-[92vh] w-[calc(100%-1.5rem)] max-w-none overflow-y-auto rounded-[28px] border-white/10 bg-[#0a0a0a] p-3 text-white sm:max-w-md sm:p-4'>
          <DialogTitle className='sr-only'>{selected.name}</DialogTitle>
          <DialogDescription className='sr-only'>{selected.quote}</DialogDescription>
          ...
          <div className='space-y-4 pr-6'>
            ...
            <div className='space-y-2'>
              <h3 className='text-2xl font-semibold leading-tight text-white'>{selected.quote}</h3>
              <p className='text-sm font-medium text-white'>{selected.name}</p>
              <p className='text-sm text-white/50'>{selected.role}</p>
            </div>
          </div>
