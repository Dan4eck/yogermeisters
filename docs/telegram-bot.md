...
Миграция `0012_neat_zarda.sql` разрешает хранить историю нескольких прохождений при единственном активном enrollment.
...
После запроса практики бот отправляет ссылку на урок йоги или запись йога-нидры с коротким текстом и кнопкой
«Хочу практиковать». «Текст5» появляется только после нажатия этой кнопки. Если запись ещё не подключена, бот сообщает
об этом и не переводит пользователя к продающему шагу.

Повторный переход по deep link отменяет прежний активный enrollment и начинает новое прохождение со вступления.
Предыдущие enrollment и deliveries сохраняются в базе как история. Внутри одного прохождения уже отправленные
сообщения и медиа автоматически повторно не выдаются.
