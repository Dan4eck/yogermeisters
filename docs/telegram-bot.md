...
| --- | --- | --- |
| Йога, видео | `PERSONAL_PRACTICE_YOGA_VIDEO_FILE_ID` | `PERSONAL_PRACTICE_YOGA_VIDEO_URL` |
| Йога-нидра, аудио | `PERSONAL_PRACTICE_NIDRA_AUDIO_FILE_ID` | `PERSONAL_PRACTICE_NIDRA_AUDIO_URL` |

Для каждой практики задаётся только один источник: file_id либо URL. Пока файла нет, оба значения
...
Для запуска потребуется предоставить видео йоги и аудио йога-нидры, настроить соответствующие указатели
в сервисе бота, применить новую миграцию и отдельно разрешить публикацию через GitHub.
