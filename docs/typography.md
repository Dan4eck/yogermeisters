...
## Система загрузочных состояний

Все брендовые загрузчики используют сплошной фон `#f8f6ec`, без видимой границы вокруг анимации и без
визуальной текстовой подписи. Состояние озвучивается скринридеру через `role="status"` и `aria-label`.

| Сценарий | Анимация | Правило |
|----------|----------|---------|
| Первый вход на главную `/` | Нет | Hero появляется сразу; глобальный загрузчик при первом рендере запрещён |
| Переход между маршрутами лендинга, кабинета и курса | Вращающаяся ваджра | Полноэкранный переход длительностью 700 мс после изменения URL |
| Загрузка данных кабинета или страницы курса | Вращающаяся ваджра | Показывается до фактического завершения API-запроса, без искусственного увеличения ожидания |
| Загрузка интро, видео или аудио внутри урока | Девочка разворачивает коврик | Показывается внутри панели до события `canplay` у медиаплеера |
| Переход по якорю внутри текущей страницы или смена языка | Нет | Это не переход между страницами |

### Ассеты и компоненты

- Ваджра: `client/public/assets/cabinet/vajra-rotation-aligned.gif`.
- Коврик: `client/public/assets/cabinet/yoga-mat-loader-beige.png`.
- Глобальные и страничные переходы: `client/src/components/VajraLoader.tsx`.
- Загрузка медиа внутри уроков: `client/src/components/YogaMatLoader.tsx`.
- Глобальная логика переходов и запрет загрузчика при первом рендере: `client/src/App.tsx`.
- Внутренние CTA должны использовать SPA-навигацию (`Link` из Wouter), иначе браузер перезагрузит приложение и
  переход с ваджрой не успеет отобразиться.
