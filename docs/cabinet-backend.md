...
Миграция создаёт пользователей, курсы, модули, уроки, отдельные права на каждый курс и таблицу PostgreSQL-сессий. Seed можно запускать повторно.
...
8. Добавьте в Railway:
   ...
   - `S3_SIGNED_URL_TTL_SECONDS`, по умолчанию `7200`.
...
6. Выйдите и убедитесь, что `/api/me` и `/api/courses` отвечают `401`.
